...
/*
* puxar alunos com patologia.
*/


  $userData	=	$db->getAllRecords('aluno','*',' AND Status = 1 AND Patologia= 1 ', 'ORDER BY Nome');
...
if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
  ...
  if(!empty($_POST['contaCheck'])){
    ...
    header('location:'.$_SERVER['PHP_SELF'].'&msg=rnna');
    exit;
  }

  if($insert){
    // mensagem add com sucesso
    header('location: presenca-turma-especial.blade.php?msg=radd');
    exit;
  }else{
    // mensagem erro
    header('location: presenca-turma-especial.blade.php?msg=rerr');
    exit;
  }
